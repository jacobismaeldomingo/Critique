// screens/watchlists/InProgressListScreen.js
...
const InProgressListScreen = ({ navigation }) => {
  ...
  const [inProgressTVSeries, setInProgressTVSeries] = useState([]);
  ...
  useFocusEffect(
    useCallback(() => {
      const loadInProgressShows = async () => {
        if (firebase_auth.currentUser) {
          ...
          setInProgressMovies(
            movies.filter((show) => show.category === "In Progress")
          );
          ...
          setInProgressTVSeries(
            tvSeries.filter((show) => show.category === "In Progress")
          );
        }
      };
      loadInProgressShows();
   ...
   */
  const filteredShows = (
    activeTab === "movies" ? inProgressMovies : inProgressTVSeries
  ).filter(
    (show) =>
